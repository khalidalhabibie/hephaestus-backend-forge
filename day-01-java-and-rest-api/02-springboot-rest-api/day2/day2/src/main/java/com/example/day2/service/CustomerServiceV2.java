...
public class CustomerServiceV2 {
    ...
    public CustomerResponse getCustomerById(Long id) {
        ...
        CustomerEntity entity = customerRepository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Customer dengan ID " + id + " tidak ditemukan"));
    ...
    private void validateRequest(CreateCustomerRequest request) {
        if (request.getFullName() == null || request.getFullName().trim().isEmpty()) throw new IllegalArgumentException("Nama Harus Diisi");
        if (request.getEmail() == null || request.getEmail().trim().isEmpty()) throw new IllegalArgumentException("Email Harus Diisi");
        if (request.getPhoneNumber() == null || request.getPhoneNumber().trim().isEmpty()) throw new IllegalArgumentException("Phone Harus Diisi");
    }
