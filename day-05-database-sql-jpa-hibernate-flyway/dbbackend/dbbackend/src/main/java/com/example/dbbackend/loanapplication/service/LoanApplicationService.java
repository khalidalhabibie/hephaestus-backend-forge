...
public class LoanApplicationService {
    ...
    public LoanApplicationResponse createLoan(CreateLoanApplicationRequest request) {
        ...
        CustomerEntity customer = customerRepository.findById(request.getCustomerId())
                .orElseThrow(
                        () -> {
                            log.warn("event=LoanCreateFailed reason=CustomerNotFound| customerId={}",
        ...
        log.info("event=LoanSubmitted loanId={} customerId={} amount={} tenor={}",
                saved.getId(),
                request.getCustomerId(),
    ...
    public LoanApplicationResponse getLoanById(Long id) {
        ...
        LoanApplicationEntity loan = loanRepository.findByIdWithCustomer(id)
                .orElseThrow(() -> {
                    log.warn("event=LoanNotFound loanId={}", id);
                    return new LoanApplicationNotFoundException("Loan not found with id: " + id);
                });
    ...
    public List<LoanApplicationResponse> getByStatus(String status) {
        log.info("LoanFilterByStatus | status={}", status);
    ...
    public LoanApplicationResponse updateStatus(Long id, LoanApplicationStatus status) {
        LoanApplicationEntity loan = loanRepository.findById(id)
                .orElseThrow(() -> {
                    log.warn("event=LoanStatusUpdateFailed reason=NotFound loanId={}", id);
                    return new LoanApplicationNotFoundException("Loan not found with id: " + id);
                });
    ...
    private void generateRepaymentSchedule(LoanApplicationEntity loan) {
        ...
        log.info("event=RepaymentGenerated loanId={} scheduleCount={}",
                loan.getId(),
                loan.getTenorMonth());
